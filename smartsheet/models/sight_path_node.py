...
class SightPathNode(PathNode):
    ...
    def _walk_to_leaf(self):
        """Yield each node from self down to the node containing the target sight."""
        node = self
        while True:
            yield node
            if node._sights:
    ...
    def get_sight_path(self):
        ...
        leaf = nodes[-1]
        if leaf._sights and leaf._sights[0].name:
            parts[-1] = leaf._sights[0].name
